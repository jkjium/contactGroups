...
# t: user threshold 
def G_Bn(data, t, varnum):
	global alphabet
	B = 5 # number of bootstrap
	count = 0
	for i in range(B):
		data_1 = bootstrap(data) # sample with replacement 
		for s in set(itertools.combinations(list(range(len(alphabet))), varnum)): # generate all variable subset with length of 2
			if T_l(list(s), data_1) >= t:
					count+=1
	#B_combnum = len(alphabet, varnum) # count B * pk
	print count
	return (1.0/B)*count
...
def threshold_t_B(data, alpha, varnum):
	global alphabet
	sk = 4
	pk = binom(len(alphabet), varnum)
	...
	for t in np.linspace(0,top,10):
		v_G = G_Bn(data, t, varnum)
		v_m = max_Tl_1(data, t, varnum)
