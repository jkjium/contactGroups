...
#                print 'matchCount trace : %d' % (matchCount)
    ...
    # pairwise
    # read res geometrical center file and kannan XXXX_A.pdb and get pairwise contact within a cutoff distance
    # output XXXX_A.res.csu_d
    # geo center file is generated by voroResidueImportGen.jar
    def pairContactbyCutoff(self, cutoff):
        fp = open(self.pdbfile+'resi.coor', 'r')
        fo = open(self.pdbfile[:-4]+'res.csu_d','w')
        cid = self.pdbfile[5]
        #for i in xrange(0, len(self.atoms)):
        coorLines = fp.readlines()
        fp.close()

        for i in xrange(0,len(coorLines)):
            a = self.atoms[i]
            line = coorLines[i].strip()
            if len(line)==0:
                continue  
            strArr = line.split(' ')
            x = strArr[1]
            ...
            for j in xrange(0, len(self.atoms)):
                ...
                if np.linalg.norm(v1-v2) <= 4.5:
                    fo.write('%s\t%d%s\t%s\t%d%s' % (a.resName, a.resSeq, cid, b.resName, b.resSeq, cid))
        fo.close()
