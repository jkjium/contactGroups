...
# mip : SDII
def report(v1,v2, cdict, rdict, pmap,mip):
	# check contact
	a1 = rdict[pmap[v1]]
	a2 = rdict[pmap[v2]]
	...
	return '%d %d %d %d %r %.6f %.4f %.4f %.4f %.4f\n' % (v1, pmap[v1], v2, pmap[v2], isContact, mip, std1, std2, std12, (std1+std2-std12))
...
def main():
	...
	with open(mipfile) as fp:
		for line in fp:
			sarr= line.split()
			idxarr = sarr[0].split('-')
			miplist.append((int(idxarr[0]), int(idxarr[1]), float(sarr[3])))
	print 'sorting mip ...'
	...
	output = ''.join([report(mip[0],mip[1], cdict, rdict, msa2resi, mip[2]/maxmip) for mip in miplist if (mip[0] in msa2resi) and (mip[1] in msa2resi)])
